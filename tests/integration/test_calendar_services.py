...
def test_calendar_service_conflict_detection():
    ...
    try:
        ...
        start2 = end1 - datetime.timedelta(hours=6)  # Overlaps with end of first reservation
        ...
        assert "⚠️ WARNING" in event2.get('description', ''), "Conflict warning not found in description"
        
        # Check reminders
        reminders = event2.get("reminders", {}).get("overrides", [])
        reminder_minutes = [r.get("minutes") for r in reminders if r.get("method") == "email"]
        assert 3 * 24 * 60 in reminder_minutes, "3-day email reminder not set for conflict event"
        assert 24 * 60 in reminder_minutes, "1-day email reminder not set for conflict event"
        ...
        for email in conflict_attendees:
            assert email in attendee_emails, f"Expected attendee email {email} not found"
