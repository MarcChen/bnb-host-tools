...
class FakeService:
    ...
    class FakeCalendarList:
        def list(self):
            return self
        def execute(self):
            return {'items': [{'summary': "Airbnb réservation | Airbnb 预订", 'id': 'primary'}]}
...
def patch_google(monkeypatch):
    ...
    monkeypatch.setattr(calendar_services, "refresh_access_token", fake_refresh_access_token)
...
def test_create_event_duplicate():
    ...
    reservation = {
        ...
        "departure_date": (datetime.datetime.now() + datetime.timedelta(hours=2)).isoformat(),
        "name": "Alice",
...
def test_create_event_success():
    ...
    assert f"Bob - NEWCODE" in svc.existing_event_summaries
